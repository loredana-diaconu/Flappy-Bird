...
(define-struct birdStruct (x y vy))
(define-struct pipe (x gap-y))
(define-struct ability (visible active))
(define-struct variables (gravity momentum scroll-speed))
(define-struct stateStruct (birdField pipes score variables))
(define initial-variables (variables initial-gravity initial-momentum initial-scroll-speed))
(define initialPipe (pipe scene-width (+ added-number (random random-threshold))))
...
(define (get-initial-state)
  (stateStruct (birdStruct bird-x bird-initial-y 0) (list (pipe scene-width (+ added-number (random random-threshold)))) 0 initial-variables))
...
(define (next-state-bird bird gravity)
  (let
      ((x (birdStruct-x bird))
       (y (+ (birdStruct-y bird) (birdStruct-vy bird))))
    (birdStruct x y (+ (birdStruct-vy bird) gravity))))
...
(define (next-state-bird-onspace bird momentum)
  (birdStruct (birdStruct-x bird) (birdStruct-y bird) (- 0 momentum)))
...
(define (change current-state pressed-key)
  (if (equal? pressed-key " ")
      (stateStruct (next-state-bird-onspace (get-bird current-state) (variables-momentum initial-variables))
                   (stateStruct-pipes current-state)
                   (stateStruct-score current-state)
                   initial-variables)
      current-state))
...
(define (move-pipes pipes scroll-speed)
  (let ((movePipe
         (lambda (p)
           (pipe (- (pipe-x p) scroll-speed) (pipe-gap-y p)))))
    (map movePipe pipes)))
...
(define (clean-pipes pipes)
  (let ((filterFunc
         (lambda (p)
           (if (> (+ (pipe-x p) (quotient scene-width 2)) 0) #t #f))))
    (filter filterFunc pipes)))
...
(define (add-more-pipes pipes)
  (if (< (length pipes) no-pipes)
      (let* ((lastPipe (last pipes)))
        (append pipes (list(pipe (+ pipe-width pipe-gap (pipe-x lastPipe)) (+ added-number (random random-threshold))))))
      pipes))
...
(define (check-ground-collision bird)
 (>= (+ (birdStruct-y bird) bird-height) ground-y))
...
(define (invalid-state? state)
  (or
       (check-ground-collision (stateStruct-birdField state))
       (check-pipe-collisions (stateStruct-birdField state) (stateStruct-pipes state))))

;(define (invalid-state? state)
  ;(if (check-ground-collision (stateStruct-birdField state)) #t #f))
...
(define (check-one-pipe bird pipe)
  (letrec ((birdX (birdStruct-x bird))
           (birdY (birdStruct-y bird))
           (pipeX (pipe-x pipe))
           (pipeGapY (pipe-gap-y pipe))
           (birdCorner1 (make-posn birdX birdY))
           (birdCorner2 (make-posn (+ birdX bird-width) (+ birdY bird-height)))
           (pipe1Corner1 (make-posn pipeX 0))
           (pipe1Corner2 (make-posn (+ pipeX pipe-width) pipeGapY))
           (pipe2Corner1 (make-posn pipeX (+ pipeGapY pipe-self-gap)))
           (pipe2Corner2 (make-posn (+ pipeX pipe-width) scene-height)))
    (or (check-collision-rectangles birdCorner1 birdCorner2 pipe1Corner1 pipe1Corner2)
        (check-collision-rectangles birdCorner1 birdCorner2 pipe2Corner1 pipe2Corner2))))

(define (check-pipe-collisions bird pipes)
    (foldl (lambda(x y) (or x y)) #f (map (lambda(pipe) (check-one-pipe bird pipe)) pipes)))
...
(define (next-state state)
  (stateStruct (next-state-bird (stateStruct-birdField state) (variables-gravity initial-variables))
               (next-state-pipes (stateStruct-pipes state) (variables-scroll-speed initial-variables))
               (+ 0.1 (stateStruct-score state))
               initial-variables))
...
(define (draw-frame state)
  (letrec ((b (stateStruct-birdField state))
        (pipesOverScene (place-pipes (stateStruct-pipes state) initial-scene))
        (scoreOverScene (place-image (score-to-image (stateStruct-score state)) text-x text-y pipesOverScene))
        (groundOverScore (place-image ground-image (quotient scene-width 2) (- scene-height (quotient ground-height 2)) scoreOverScene))
        (birdOverGround (place-image bird-image (+ (birdStruct-x b) (quotient bird-width 2)) (+ (birdStruct-y b) (quotient bird-height 2)) groundOverScore)))
    birdOverGround))

; Folosind `place-image/place-images` va poziționa pipe-urile pe scenă.

(define (get-pos-list-pipes pipes)
  (let ((one-pipe (lambda (p)
                    (make-posn (+ (pipe-x p) (quotient pipe-width 2)) (+ 0 (quotient scene-height 2))))))
    (map one-pipe pipes)))

(define (get-pos-list-gaps pipes)
  (let ((one-gap (lambda (p)
                    (make-posn (+ (pipe-x p) (quotient pipe-width 2)) (+ (pipe-gap-y p) (quotient pipe-self-gap 2))))))
    (map one-gap pipes)))

(define (place-pipes pipes scene)
  (letrec ((pipe-pos (get-pos-list-pipes pipes))
           (gap-pos (get-pos-list-gaps pipes))
           (pipe-image-list (make-list (length pipes) pipe-image))
           (gap-image-list (make-list (length pipes) pipe-gap-image))
           (pipes-only (place-images pipe-image-list pipe-pos scene))
           (res (place-images gap-image-list gap-pos pipes-only)))
    res))
